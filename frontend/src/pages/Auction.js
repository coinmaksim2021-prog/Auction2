...
function Auction() {
  ...
  // Auction stats
  const [currentBid] = useState(850);
  const [totalBids] = useState(137);
  const [participants] = useState(89);
  const [onlineUsers] = useState(42);
  const [bidsLastHour] = useState(23);
  const [ogTrailblazers] = useState(94);
  ...
  useEffect(() => {
    const hints = [
      { user: '0x7a3...f2d', amount: '1200 USDC', rarity: 'Rare' },
      { user: '0x9c1...a8b', amount: '950 USDC', rarity: 'Uncommon' },
      { user: '0x4e2...c7f', amount: '1800 USDC', rarity: 'Epic' },
      { user: '0x2b5...d9e', amount: '750 USDC', rarity: 'Uncommon' },
      { user: '0x8f4...b3a', amount: '2100 USDC', rarity: 'Legendary' }
    ];
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-gray-50 to-gray-100" data-testid="auction-page">
      ...
      <div className="relative max-w-7xl mx-auto px-4 py-8">
        ...
        {/* Last Chance - Last Hero Block (Visual Separator) */}
        ...
        <div className="fixed bottom-6 left-6 bg-white rounded-xl shadow-lg border border-gray-200 p-4 max-w-sm z-50">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-600">{currentHint.user} bid {currentHint.amount}</p>
            </div>
          </div>
        </div>
      )}
      ...
      {/* Place Bid Modal */}
      {showBidModal && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50" onClick={() => setShowBidModal(false)}>
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <button
                    ...
                    className={`p-3 rounded-xl border text-left transition-all ${bidAmount === '100' ? 'border-emerald-500 bg-emerald-50' : 'border-gray-200 hover:border-gray-300'}`}
                  >
                    <p className="font-semibold text-gray-900">100 USDC</p>
                    <p className="text-xs text-gray-500">Minimum bid</p>
                  </button>
                  <button
                    onClick={() => setBidAmount('200')}
                    className={`p-3 rounded-xl border text-left transition-all ${bidAmount === '200' ? 'border-emerald-500 bg-emerald-50' : 'border-gray-200 hover:border-gray-300'}`}
                  >
                    <p className="font-semibold text-gray-900">200 USDC</p>
                    <p className="text-xs text-emerald-600">+5% Rarity Boost</p>
                  </button>
                  <button
                    onClick={() => setBidAmount('300')}
                    className={`p-3 rounded-xl border text-left transition-all ${bidAmount === '300' ? 'border-emerald-500 bg-emerald-50' : 'border-gray-200 hover:border-gray-300'}`}
                  >
                    <p className="font-semibold text-gray-900">300 USDC</p>
                    <p className="text-xs text-emerald-600">+10% Rarity Boost</p>
                  </button>
                  <button
                    onClick={() => setBidAmount('500')}
                    className={`p-3 rounded-xl border text-left transition-all ${bidAmount === '500' ? 'border-emerald-500 bg-emerald-50' : 'border-gray-200 hover:border-gray-300'}`}
                  >
                    <p className="font-semibold text-gray-900">500 USDC</p>
                    <p className="text-xs text-emerald-600">+20% Rarity + Early Bird</p>
                  </button>
                </div>
              </div>
